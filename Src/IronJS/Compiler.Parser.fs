...
module Parser =
  ...
  /// Raises an error that the current 
  /// token is of an unexpected type
  let unexpectedToken (p:P) = p |> P.unexpectedToken p.Token
  ...
  let try' _ p =
    ...
    | S.Catch -> 
      p |> consume                        // catch
      p |> expect S.LeftParenthesis       // (
      let name = p |> consumeIdentifier   // identifier
      p |> expect S.RightParenthesis      // )
      let catch = 
        [Tree.Catch(name, p |> block)]    // { [statements] }
  ...
  let objectLiteral _ p =
    ...
    while p |> csymbol <> S.RightBrace do
        ...
        let property = Tree.Assign(name, value)
  ...
  let parseGlobalSource env source = 
    Tree.Function(None, Scope.NewGlobal, parse source env)