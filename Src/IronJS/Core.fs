...
//
//-------------------------------------------------------------------------
and [<AllowNullLiteral>] StaticFunction =
  inherit Function

  val mutable Method : MethodInfo
  val mutable ArgTypes : System.Type array
  val mutable ReturnType : System.Type
  val mutable GenericArgs : System.Type array

  new (env:IjsEnv, method':MethodInfo) = {
    inherit Function(env, env.Base_Class)
    Method = method'
    ArgTypes = [|for x in method'.GetParameters() -> x.ParameterType|]
    ReturnType = method'.ReturnType
    GenericArgs = method'.GetGenericArguments()
  }
    
  member x.isGeneric = 
    x.GenericArgs.Length > 0
