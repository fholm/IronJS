...
let private compileDynamicAst (ctx:Context) (body:Et list) = 
  ...
  let completeBodyExpr = 
    innerParameters 
      |> Map.fold (fun state _ (var:Ast.Local) -> makeDynamicInitExpr var argsArray :: state) body
...
let compileAst (closureType:ClrType) (scope:Ast.Scope) (ast:Ast.Node) =
  let context = {Context.New with Closure = Dlr.Expr.param "~closure" closureType; Scope = scope; Builder = Compiler.ExprGen.builder}
  let body    = [(Compiler.ExprGen.builder context ast); Dlr.Expr.labelExpr context.Return]

  if scope.CallingConvention = Ast.CallingConvention.Dynamic 
